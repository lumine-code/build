...
module.exports = {
  ...
  async refreshTargets({ notify = false, updateList = true } = {}) {
    ...
    if (!context.projectPath) {
      ...
      if (updateList) this.selectList.setItems([]);
      return [];
    }
    ...
    try {
      ...
      this.activeTarget =
        ...
        this.targets[0] ??
        null;
      if (updateList) this.selectList.setItems(this.targets);
      this.syncTargetCommands();
  ...
  async runActiveTarget() {
    ...
    if (lumine.config.get("build.saveOnBuild")) {
      await Promise.all(
        lumine.workspace
          .getTextEditors()
          .filter((editor) => editor.getFileState() !== FileState.UNMODIFIED)
