...
module.exports = {
  activate() {
    ...
    this.statusElement = document.createElement("button");
    this.statusElement.className = "build-status inline-block";
    ...
    this.subscriptions.add(
      ...
      lumine.commands.add("lumine-workspace", {
        "build:trigger": () => this.runActiveTarget(),
        "build:stop": () => this.stop(),
        "build:toggle-panel": () => this.panel.toggle(),
        "build:clear": () => this.panel.clear(),
        "build:select-target": () => this.selectTarget(),
        "build:refresh-targets": () => this.refreshTargets({ notify: true }),
        "build:next-error": () => this.openError(1),
        "build:previous-error": () => this.openError(-1),
      }),
